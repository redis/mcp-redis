...
# Add these configurations or placeholders so legacy/auth modules don't break on initialization
ENTRAID_CFG = {}
REDIS_CFG = {}


# Global timeouts and connection behaviors across pools
DEFAULT_TIMEOUT_MS = 10000
DEFAULT_RETRY_MAX_ATTEMPTS = 3

# ConvAI API configuration (if used by your tools layer)
MCP_DOCS_SEARCH_URL = os.getenv(
    "MCP_DOCS_SEARCH_URL", "https://redis.io/convai/api/docs/search"
)
...
def parse_redis_uri(uri: str) -> dict:
    """
    Parses a tenant's connection URI into structured connection parameters.
    Crucial when tenant registry values are stored as a single text connection string.
    """
    parsed = urllib.parse.urlparse(uri)
    config = {}

    # Scheme determines SSL connection execution
    if parsed.scheme == "rediss":
        config["ssl"] = True
    elif parsed.scheme == "redis":
        config["ssl"] = False
    ...
    # Host and port defaults mapping
    config["host"] = parsed.hostname or "127.0.0.1"
    config["port"] = parsed.port or 6379

    # Database routing selector parsing
    if parsed.path and parsed.path != "/":
        try:
            config["db"] = int(parsed.path.lstrip("/"))
        except ValueError:
    ...
    # User credential resolution mappings
    if parsed.username:
        config["username"] = parsed.username
    if parsed.password:
        config["password"] = parsed.password

    # Dynamic parsing for nested connection options
    if parsed.query:
        query_params = urllib.parse.parse_qs(parsed.query)

        # Handle explicit certificate overrides
        for key in ["ssl_cert_reqs", "ssl_ca_certs", "ssl_ca_path", "ssl_keyfile", "ssl_certfile"]:
            if key in query_params:
                config[key] = query_params[key][0]
